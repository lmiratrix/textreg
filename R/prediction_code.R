...
#' Refit the labels to the selected phrases to get non-shrunk predictions.
#' (Uses logistic fit rather than linear, as well.)
#'
#' @param model The textreg.result object from a text regression
#' @param method Either "glm" or "ols".  Type of model to fit.
#' @return A fitted regression model
#'
#' @export
refit.model = function( model, method=c( "glm", "ols" ) ) {

    method = match.arg( method )

    X = as.data.frame( phrase.matrix(model) )
    X

    X$L = (model$labeling + 1)/2
    M.refit = NULL
    if ( method == "glm" ) {
        M.refit = glm( L ~ 0 + ., data=X, family = "binomial")
    } else {
        M.refit = lm( L ~ 0 + ., data=X )
    }
    M.refit
}
